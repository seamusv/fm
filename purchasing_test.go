...
func TestGeneratePurchaseOrderNumber(t *testing.T) {
	...
	executor.On("ExecuteFields", "ADD", mock.AnythingOfType("PO401"), "Z00062").Return(
		func(command string, screen interface{}, expectedCodes ...string) *fm.Response {
	...
	executor.On("Execute", "PROCESS", "P40163").Return(
		func(command string, expectedCodes ...string) *fm.Response {
			r, err := fm.Parse(buildMockXMLResponse(map[string]string{"IDORDR": "C00006942"}))
			assert.NoError(t, err)
			return r
		})
	...
	output, err := fm.GeneratePurchaseOrderNumber(processor, buildDate(2017, time.December, 24), input)
	assert.NoError(t, err)
