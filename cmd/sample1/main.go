...
import (
	...
	"github.com/seamusv/fm-integration/executor.http"
	"log"
	"os"
	"sync"
	"time"
...
func main() {
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			defer wg.Done()
			output, err := fm.GeneratePurchaseOrderNumber(processor, time.Now(), input)
			if err != nil {
				log.Fatal(err)
			}
...
func (m *MyProcessor) Process(f func(fm.Executor)) {
	m.jobCh <- f
}
